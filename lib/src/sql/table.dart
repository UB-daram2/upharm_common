...
class ChangeSet {
  ...
  Map toEncodable() => {"tablename": tablename, "inserted": inserted, "deleted": deleted, "updated_new": updatedNew, "updated_old": updatedOld};
}
...
class Table {
  ...
  bool get modified {
    ...
    return (chg.inserted.isNotEmpty || chg.deleted.isNotEmpty || chg.updatedNew.isNotEmpty);
  }
  ...
  Map<String, dynamic> newRow() {
    ...
    for (int t = 0; t < columns.length; t++) {
      newRow[columns[t].columnName] = columns[t].allowDBNull ? null : _defaultValue(columns[t].dataTypeName);
    }
    return newRow;
  }
  ...
  Future post() async {
    ...
    if (chg.inserted.isEmpty && chg.deleted.isEmpty && chg.updatedNew.isEmpty) return postCompleter.future;
  ...
  ChangeSet _detectChanges() {
    ...
    for (var t = 0; t < rows.length; t++) {
      ...
      if (!_areRowEquals(currentRow, originalRow)) {
        ...
        var fields = List.from(cr.keys); // copy to a new list to avoid concurrent cancellation
  ...
  dynamic _defaultValue(String type) {
    Map def = {"bit": false, "varchar": "", "int": 0, "smallint": 0, "decimal": 0.0, "float": 0.0, "datetime": DateTime.parse("1899-12-30T12:00:00")};
