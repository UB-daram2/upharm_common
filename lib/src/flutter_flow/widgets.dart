...
class WgButtonOptions {
  const WgButtonOptions({
    this.textStyle,
    this.elevation,
    this.height,
    this.width,
...
class WgButtonWidget extends StatefulWidget {
  const WgButtonWidget({
    super.key,
    required this.text,
  ...
  final WgButtonOptions options;
  final bool showLoadingIndicator;

  @override
  State<WgButtonWidget> createState() => _WgButtonWidgetState();
}

class _WgButtonWidgetState extends State<WgButtonWidget> {
  bool loading = false;
  ...
  Widget build(BuildContext context) {
    Widget textWidget = loading
        ? SizedBox(
            width: widget.options.width == null ? _getTextWidth(widget.text, widget.options.textStyle, maxLines) : null,
            child: Center(
              child: SizedBox(
                width: 23,
                height: 23,
...
double _getTextWidth(String text, TextStyle? style, int maxLines) => (TextPainter(
